...
(*s https://doc.rust-lang.org/reference/keywords.html *)
...
let pp_tvar id = str ("TYPE_VARIABLE_" ^ Id.to_string id)

let pp_abst = function
  | [] -> mt ()
  | l  ->
      str "|" ++ prlist_with_sep (fun () -> str ", ") Id.print l ++
      str "|" ++ spc ()
...
let pp_tdummy usf =
  if usf.tdummy || usf.tunknown then str "type _SOME_PROPOSITION = Obj.t" ++ fnl () else mt ()

let pp_mldummy usf =
  if usf.mldummy then
    str "let _SOME_PROPOSITION = let rec f _ = Obj.repr f in Obj.repr f" ++ fnl ()
  else mt ()
